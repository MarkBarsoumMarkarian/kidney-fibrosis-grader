...
class fpn(nn.Module):
    ...
    def _crop_global(self, f_global, top_lefts, ratio):
        ...
        for i in range(b):
            ...
            if left >= W: left = W-1
            top_end = min(top + h, H)
            left_end = min(left + w, W)
            f_global_patch = f_global[0:1, :, top:top_end, left:left_end]
            # always resize to exact (h, w) so all crops in batch are identical size
            f_global_patch = torch.nn.functional.interpolate(f_global_patch, size=(h, w), mode='bilinear', align_corners=False)
